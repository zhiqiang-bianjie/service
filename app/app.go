...
var (
	...
	maccPerms = map[string][]string{
		...
		service.DepositAccName:          {auth.Burner},
...
func NewSimApp(
	...
	app.ServiceKeeper = service.NewKeeper(appCodec, keys[service.StoreKey], app.AccountKeeper, app.BankKeeper,
		service.MockTokenKeeper{}, app.subspaces[service.ModuleName], auth.FeeCollectorName)
	...
	app.mm = module.NewManager(
		...
		transferModule,
		service.NewAppModule(app.ServiceKeeper, app.AccountKeeper, app.BankKeeper),
	)
	...
	app.mm.SetOrderBeginBlockers(
		...
		evidence.ModuleName, staking.ModuleName, ibc.ModuleName, service.ModuleName,
	)
	app.mm.SetOrderEndBlockers(crisis.ModuleName, gov.ModuleName, staking.ModuleName)

	// NOTE: The genutils moodule must occur after staking so that pools are
	// properly initialized with services from genesis accounts.
...
// BlacklistedAccAddrs returns all the app's module account addresses black listed for receiving services.
